...
if[system"t";
 .z.ts:{pub'[t;value each t];@[`.;t;@[;`sym;`g#]0#];i::j;ts .z.D;.cron.run[]};
 upd:{[t;x]
 if[not -16=type first first x;if[d<"d"$a:.z.P;.z.ts[]];a:"n"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 t insert x;if[l;l enlist (`upd;t;x);j+:1];cnt[t]+:count first x;}];

if[not system"t";system"t 1000";
 .z.ts:{ts .z.D;.cron.run[]};
 upd:{[t;x]ts"d"$a:.z.P;
 //if[not -16=type first first x;a:"n"$a;x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
 f:cols value t;pub[t;rs:$[0>type first x;enlist f!x;flip f!x]];if[l;l enlist (`upd;t;rs);i+:1];cnt[t]+:count first x;}];

logTickInf:{.log.out "\nNumber of messages processed so far:\n",.Q.s .u.cnt;.log.out "\nLatest subscriber details:\n",.Q.s .u.w};
\d .
.u.tick["tp_";.env.tplogDir];
if[.u.i;upd:{[x;y] .u.cnt[x]+:count  y};-11!.u.L;delete upd from `.];

.cron.add[`.u.logTickInf;(::);.z.P;0Wp;1000*60];
